                ...
                <div class="relative" data-more-actions-toggle>
                    ...
                    <div v-if="showMoreActionsDropdown" class="absolute right-0 mt-2 w-48 bg-neutral-800 border border-neutral-700 rounded-md shadow-lg z-10">
                        <div class="py-1">
                            ...
                            <button
                                @click="bulkUpdateImages"
                                :disabled="bulkImageUpdateLoading"
                                class="w-full px-4 py-2 text-left text-sm text-white hover:bg-neutral-700 flex items-center disabled:opacity-50"
                            >
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                                </svg>
                                {{ bulkImageUpdateLoading ? 'Updating images...' : 'Update Missing Images' }}
                            </button>
                            <button
...
const bulkImageUpdateLoading = ref<boolean>(false);

const bulkUpdateImages = async (): Promise<void> => {
    try {
        bulkImageUpdateLoading.value = true;
        showMoreActionsDropdown.value = false;
        showNotification('info', 'Updating missing images from article pages...');

        const response = await feedClient.raw.bulkUpdateImages();

        if (response && response.success) {
            showNotification('success', `Images updated: ${response.updated} updated, ${response.failed} not found, ${response.skipped} already had image`);
            await refreshData();
        } else {
            throw new Error('Failed to update images');
        }
    } catch (err: unknown) {
        showNotification('error', err instanceof Error ? err.message : 'Failed to update images');
    } finally {
        bulkImageUpdateLoading.value = false;
    }
};
