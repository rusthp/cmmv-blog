...
async function resolveImage(url, title) {
    // ---------------------------------------------------------
...
async function createAndSavePlaceholder(title) {
    try {
...
async function runTests() {
    let logString = "=== STARTING IMAGE FALLBACK TESTS ===\n";
    ...
    for (const test of tests) {
        ...
        try {
            ...
            logString += `Result:` + JSON.stringify(result, null, 2) + `\n`;
        } catch (e) {
            logString += `Error: ${e.message}\n`;
        }
    }

    fs.writeFileSync(path.join(__dirname, 'resolver-test-output.txt'), logString);
}

runTests().catch(e => fs.writeFileSync(path.join(__dirname, 'resolver-test-error.txt'), e.toString()));
