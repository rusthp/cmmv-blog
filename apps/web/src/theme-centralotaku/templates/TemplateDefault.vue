<template>
    <div class="flex min-h-screen bg-gray-100">
        ...
        <button
            ...
            class="fixed top-4 left-4 z-50 p-2 bg-black text-white rounded-md shadow-md focus:outline-none lg:hidden cursor-pointer"
        >
        ...
        <!-- Sidebar -->
        <aside
            class="fixed inset-y-0 left-0 w-64 border-r border-red-600 bg-black z-40 transform transition-transform duration-300 ease-in-out overflow-y-auto"
            ...
            <div class="h-full flex flex-col">
                ...
                <div class="p-6 border-b border-red-600">
                    <a href="/" class="flex flex-col items-center mb-4">
                        <img src="/src/theme-centralotaku/logo2.png" width="130" height="40" alt="Logo" title="Logo">
                    </a>
                    ...
                    <div class="flex items-center justify-center gap-4 mb-4">
                        <a v-if="settings['blog.facebook']" :href="`https://facebook.com/${settings['blog.facebook']}`" target="_blank" rel="noopener noreferrer" title="Facebook"
                            class="text-white hover:text-red-500 transition-colors p-2 rounded-md hover:bg-red-600/10 sidebar-icon-hover">
                    ...
                    <div class="flex items-center justify-center gap-2">
                        <!-- Botão de Tema -->
                        <button @click="toggleTheme" class="text-white hover:text-red-500 transition-colors p-3 rounded-lg hover:bg-red-600/10 flex items-center justify-center sidebar-icon-hover" :title="isDarkMode ? 'Modo Claro' : 'Modo Escuro'">
                        ...
                        <!-- Botão de Pesquisa -->
                        <button @click="openSearchModal" class="text-white hover:text-red-500 transition-colors p-3 rounded-lg hover:bg-red-600/10 flex items-center justify-center sidebar-icon-hover" title="Pesquisar" aria-label="Pesquisar">
                ...
                <div class="flex-1 overflow-y-auto p-6">
                    <h2 class="text-lg font-semibold mb-6 text-white">Categorias</h2>
                    <ul class="space-y-1">
                        <template v-for="category in mainNavCategories.rootCategories" :key="category.id">
                            <li v-if="mainNavCategories.childrenMap[category.id]" class="w-full">
                                <button
                                    @click="(e) => toggleDropdown(category.id, e)"
                                    class="dropdown-toggle flex items-center justify-between w-full text-white hover:bg-red-600 hover:text-white rounded-lg px-4 py-3 text-sm font-medium transition-colors"
                                    :class="{'bg-red-600 text-white': openDropdowns[category.id]}"
                                >
                                    {{ category.name }}
                                    <svg class="w-4 h-4 transition-transform duration-200" :class="{'rotate-180': openDropdowns[category.id]}" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
                                    </svg>
                                </button>
                                <div v-show="openDropdowns[category.id]" class="ml-4 mt-2 space-y-1">
                                    <a
                                        v-for="child in mainNavCategories.childrenMap[category.id]"
                                        :key="child.id"
                                        :href="`/category/${child.slug}`"
                                        class="block px-4 py-2 text-sm text-gray-300 hover:bg-red-500 hover:text-white rounded-lg transition-colors"
                                    >
                                        {{ child.name }}
                                    </a>
                                </div>
                            ...
                            <a
                                :href="`/category/${category.slug}`"
                                    class="block text-white hover:bg-red-600 hover:text-white rounded-lg px-4 py-3 text-sm font-medium transition-colors"
                            >
                                {{ category.name }}
                            </a>
                            </li>
                ...
                <!-- Footer da Sidebar -->
                <div class="p-6 border-t border-red-600 bg-black flex justify-center">
                    <a href="/terms-of-service" class="px-3 py-2 text-xs text-gray-400 hover:text-red-500 transition-colors rounded-md hover:bg-red-600/10">
                        Termos de Uso
                        </a>
                    <span class="text-gray-600 self-center mx-2">|</span>
                    <a href="/terms-of-privacy" class="px-3 py-2 text-xs text-gray-400 hover:text-red-500 transition-colors rounded-md hover:bg-red-600/10">
                        Privacidade
                    </a>
                </div>
            </div>
...
/* Melhorias para os ícones e botões da sidebar */
.sidebar-icon-hover {
