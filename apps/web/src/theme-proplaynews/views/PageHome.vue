            ...
            <div class="flex flex-col lg:flex-row gap-8">
                <div class="flex-grow">
                    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        <div class="lg:col-span-2">
                            ...
                            {{ post.excerpt || (stripHtml(post.content).length > 40 ? stripHtml(post.content).substring(0, 40) + '...' : stripHtml(post.content)) }}
                        </p>
                ...
                <div v-if="adStates.showInContentAd" class="w-full bg-gray-100 rounded-lg my-8 overflow-hidden flex justify-center">
                            ...
                            <div v-if="hasMorePosts && !loadingMore" class="mt-8 flex justify-center">
                                <button 
                                    @click="loadMorePosts"
                                    class="bg-gradient-to-r from-purple-600 to-purple-700 hover:from-purple-700 hover:to-purple-800 text-white font-semibold px-8 py-3 rounded-lg transition-all duration-300 shadow-lg hover:shadow-xl transform hover:-translate-y-1 focus:outline-none focus:ring-4 focus:ring-purple-300 focus:ring-opacity-50"
                                >
                                    Ver Mais Posts
                                </button>
                            </div>
                            ...
                            <div v-if="!hasMorePosts && posts.length > 0" class="mt-8 text-center py-4 text-gray-500">
...
const loadPosts = async () => {
    try {
        ...
        // Usar offset 0 para carregamento inicial
        const response: any = await blogAPI.posts.getAll({ offset: 0 });

        if (response && response.posts) {
            posts.value = response.posts;

            // Atualizar paginação baseada na resposta da API
            pagination.value = {
                total: response.count || 0,
                limit: 32, // Sincronizar com o limite da API
                offset: response.posts.length
            };

            // Calcular se há mais posts baseado no total vs quantidade atual
            hasMorePosts.value = response.posts.length < response.count;
        ...
        if (response && response.posts && response.posts.length > 0) {
            ...
            if (newPosts.length > 0) {
                ...
                pagination.value = {
                    total: response.count || 0,
                    limit: 32,
                    offset: pagination.value.offset + newPosts.length
                };

                // Recalcular se há mais posts
...
// Função setupIntersectionObserver removida - agora usando botão "Ver mais"
...
// Inicializar posts automaticamente
...
onMounted(async () => {
    ...
    // Carregar posts se não houver posts iniciais
    if (!posts.value || posts.value.length === 0) {
        await loadPosts();
    }
